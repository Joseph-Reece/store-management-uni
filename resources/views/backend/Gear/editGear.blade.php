                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title">Edit Gear </h4>
                                ...
                                <div class="card-content collapse show">
                                    <div class="card-body card-dashboard ">
                                        ...
                                        @endif
                                        <form class="form" method="POST" action="{{ route('gear.update', $gear->id) }}"
                                            enctype="multipart/form-data">
                                            @method('PUT')
                                            @csrf
                                            <div class="form-body">
                                                <div class="col-md-12">
                                                    <img class="dropzone" src="/uploads/{{$gear->image }}" alt="" id="previewImg">

                                                    <input type="file" name="file" value="{{$gear->image}}" class="form-control-file" id="exampleInputFile" onchange="previewFile(this);">
                                                </div>
                                                <div class="row">
                                                            ...
                                                            <select class="select2 form-control block" id="sport"
                                                                ...
                                                                <optgroup label="Sports List">
                                                                    @foreach ($sports as $key => $sport)
                                                                        <option value={{ $key }} {{$gear->sport == $key ? 'selected' : ''}}>
                                                                            {{ $sport }} </option>
                                                                    @endforeach
                                                                </optgroup>
                                                            </select>
                                                            ...
                                                            <select class="select2 select form-control block" id="category"
                                                                ...
                                                                <optgroup label="Categories">
                                                                    @foreach ($categories as $key => $category)
                                                                        <option value={{ $key }} {{$gear->category == $key ? 'selected' : ''}}>
                                                                            {{ $category }} </option>
                                                                    @endforeach
                                                                </optgroup>
                                                            </select>
